...
function createEntryFile (type, entryDir) {
  ...
  // 获取主题文件夹中的scss文件
  const files = (fs.readdirSync(themeSrcDir) || []).filter(item => item.includes('.scss'))
...
function getEntry (files, entryDir) {
  const entry = []
  files.forEach(file => {
    const name = file.replace('.scss', 'js')
...
module.exports = (type = 'el', callback) => {
  filesWriteToIndex()
